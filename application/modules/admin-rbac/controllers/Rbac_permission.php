...
class Rbac_permission extends CI_Controller {
    ...
    public function index(){
        ...
        $this->db->select('rbac_menu.*,rbac_menu.id_menu AS id');
        $this->db->order_by('rbac_menu.weight','ASC');
        $mini_list = $this->db->get('rbac_menu')->result_array();
        $data['combo_menu'] = $this->global_model->general_combo_tree($mini_list,'yes','yes','yes','0','0','text',array());
    ...
    function show_permission(){
        $data_perm = $this->global_model->general_select('rbac_permissions',array('`perm_id`'=>$_POST['id']),'row','',array());

...
class Model_ruangan extends CI_Model
{
    function get_query_string_1($data,$columns){
        ...
        $query_string = "
            ...
            FROM tb_ruangan a
			left join rbac_user b on b.id=a.id_pegawai 
            WHERE (1=1)
                AND a.`flag_delete`='0'
                AND ".$q."
                AND ".$q_search."
            GROUP BY a.id_ruangan";
	        return $query_string;
    }
    ...
    function get_data_1($data,$sortcol,$columns) {
        ...
        $query = $db_->query($this->get_query_string_1($data,$columns)."
            ORDER BY a.nama_ruangan DESC, a.created_date DESC LIMIT ".$data['iDisplayStart'].",".$data['iDisplayLength']);
        // echo $db_->last_query();
        return $query->result();
    }

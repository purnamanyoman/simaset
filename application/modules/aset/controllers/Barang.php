...
class barang extends CI_Controller {
    ...
    public function index(){
        ...
        $data['url_load_bid']=base_url()."aset/barang/load_bid";

        $data['url_load_kel']=base_url()."aset/barang/load_kel";
		$data['url_load_skel']=base_url()."aset/barang/load_skel";
    ...
    public function list_data(){
		...
		$html .= '
                [
                    ...
                    "<span '.$css_span1.'><div '.$css_nowrap.'>Nama Barang : '.  $row->ur_sskel.'<br>Kode Aset :  '.$row->kd_brg.'   </span>",
                ...
                // [
                    // "<input type=\"checkbox\" name=\"check_list\" value=\"'.$row->id.'\">",
                    // "<span '.$css_span1.'><div '.$css_nowrap.'>Nama Barang : '.  $row->ur_sskel.'<br>Kode Aset :  '.$row->kd_brg.' <br> Nama golongan : '.$row->kd_gol.'<br>Nama kelompok : '.$row->kd_kel.'<br>Nama sub kelompok :'.$row->kd_skel.' </span>",
                    // "<button type=\"button\" class=\"btn btn-primary btn-flat btn-sm btn_table_edit\" onclick=\"show_aset('.$row->id.');\" title=\"view / edit\"><i class=\"fa fa-fw fa-edit\"></i></button>"
                // ],';
                $no++;
            }
    ...
    function add_data(){
        ...
        if ($data_id =='') {
			...
			if($this->model_barang->getKodeAda($_POST['kd_brg'])<=0)				
			{				
    ...
    function show_aset(){
		...
		$arr['kd_bid']=  $data_aset->kd_bid; 
		$arr['kd_kel']=  $data_aset->kd_kel; 
		$arr['kd_skel']=  $data_aset->kd_skel;
		$arr['kd_sskel']=  $data_aset->kd_sskel; 	
	...
	function load_bid(){
            ...
            $bid = $this->global_model->general_select('t_bid',array('kd_gol'=>$_POST['kd_gol']),'result','',NULL,'default');

            echo $this->global_model->general_combo($bid,'yes','kd_bid','ur_bid','');
    ...
    function load_kel(){
        ...
        $kel = $this->global_model->general_select('t_kel',array('`kd_gol`'=>$_POST['kd_gol'],'kd_bid'=>$_POST['kd_bid']),'result','','','default');

        echo $this->global_model->general_combo($kel,'yes','kd_kel','ur_kel','');

    }

     function load_skel(){

        $skel = $this->global_model->general_select('t_skel',array('`kd_gol`'=>$_POST['kd_gol'],'kd_bid'=>$_POST['kd_bid'],'kd_kel'=>$_POST['kd_kel']),'result','','','default');

        echo $this->global_model->general_combo($skel,'yes','kd_skel','ur_skel','');

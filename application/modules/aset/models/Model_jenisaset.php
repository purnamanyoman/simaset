...
class Model_jenisaset extends CI_Model
{
    function get_query_string_1($data,$columns){
        ...
        $query_string = "
            SELECT tb_jenis_aset.*
            FROM tb_jenis_aset
            WHERE (1=1)
                AND tb_jenis_aset.`flag_delete`='0'
                AND ".$q."
                AND ".$q_search."
            GROUP BY tb_jenis_aset.id_jenisaset";
    ...
    function get_data_1($data,$sortcol,$columns) {
        ...
        $query = $db_->query($this->get_query_string_1($data,$columns)."
            ORDER BY nama_jenisaset DESC, created_date DESC LIMIT ".$data['iDisplayStart'].",".$data['iDisplayLength']);
        // echo $db_->last_query();
        return $query->result();
    }

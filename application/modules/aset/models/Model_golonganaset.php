...
class Model_golonganaset extends CI_Model
{
    function get_query_string_1($data,$columns){
        ...
        $query_string = "
            SELECT tb_golongan.*,tb_jenis_aset.nama_jenisaset
            FROM tb_golongan,tb_jenis_aset
            WHERE (1=1)
                AND tb_golongan.`flag_delete`='0'
                AND tb_golongan.`id_jenisaset`=tb_jenis_aset.`id_jenisaset`
                AND ".$q."
                AND ".$q_search."
            GROUP BY tb_golongan.id_golongan";
    ...
    function get_data_1($data,$sortcol,$columns) {
        ...
        $query = $db_->query($this->get_query_string_1($data,$columns)."
            ORDER BY nama_golongan DESC, created_date DESC LIMIT ".$data['iDisplayStart'].",".$data['iDisplayLength']);
        // echo $db_->last_query();
        return $query->result();
    }

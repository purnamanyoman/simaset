<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('.datetimepicker_bulan').datepicker({
            startView: 'months',
            minViewMode: 'months',
            format: 'mm',
            autoclose: true,
        ...
        $('#role_active input[type="radio"]').change(function(){
            $.ajax({
                ...
                success:function(data){
                    ...
                    $.blockUI({ message: "<h4>sistem sedang mengganti hak akses anda...</h4>" });
                    if(data.submit=='1'){
        ...
        $('.dataTables_filter label').css('float','none').css('width','98.6%');
        ...
        $('#tahun_ajar').change(function(){
            $.ajax({
                url:'<? echo base_url()."admin-rbac/rbac_user/change_tahun_ajar_active"?>',
                type: 'POST',
                data:"x_value="+$('#tahun_ajar').val()+"&x=tahun_ajar_active",
                dataType: 'json',
                success:function(data){
                    catch_expired_session(data);
                    // $.blockUI({ message: "<h4>system was changing Tahun Ajar...</h4>" });
                    if(data.submit=='1'){
                        // setTimeout($.unblockUI, 3000);
                        return true;
                    }
    ...
    function clear_form(form_id){
        if ( $(form_id+" .form-input-data").is( "form" ) ) {
            ...
            $(form_id+" .form-input-data .input_select").trigger('change.select2');
    ...
    function logout(){
        swal({
            title: "Anda yakin akan keluar dari sistem ?",
            text: "sistem akan menghapus session anda.",
            type: "warning",
        ...
        function(isConfirm){
            if (isConfirm){
                window.location.href = '<?php echo base_url()."admin-authentication/logout"?>';
            }
            else{
                return false;
            }
